/**
 * MIGRATION: Create user_companies table
 * 
 * This migration creates the user_companies junction table with:
 * - User and company foreign keys
 * - Role information for each user within a company
  ...
  try {
    await sequelize.query(`
      CREATE TABLE IF NOT EXISTS user_companies (
        ...
        role VARCHAR(20) NOT NULL DEFAULT 'employee' CHECK (role IN ('owner', 'admin', 'manager', 'accountant', 'hr', 'employee', 'viewer')),
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (command === 'up') {
    ...
    console.log('Usage: node create_user_companies_table.js [up|down]');
    process.exit(1);
  }
}
